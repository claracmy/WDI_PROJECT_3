angular.module('whatsOn').controller('filesNewCtrl', filesNewCtrl);
...
function filesNewCtrl(
  ...
  vm.pickFile = () => {
    filepickerService.pick({ mimetype: 'text/*' }, Blob => {
      if (Blob && Blob.url) {
        vm.file.content = Blob.url;
        $http.get(Blob.url).then(response => {
  ...
  function textFileSubmit() {
    ...
    File.save(vm.file).$promise.then(() => {
      $state.go('filesIndex');
    });
  }
}
